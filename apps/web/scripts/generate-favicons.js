...
/**
 * Creates a valid multi-image ICO buffer from PNG buffers.
 * Conforms to ICO specification (ICONDIR + ICONDIRENTRY[] + PNG images).
 */
function createIco(images) {
  ...
  // 1. ICONDIR Header
  out.writeUInt16LE(0, 0); // Reserved. Always 0.
  out.writeUInt16LE(1, 2); // Image type: 1 for icon (.ICO).
  out.writeUInt16LE(count, 4); // Number of images in file.
  ...
  for (let i = 0; i < count; i++) {
    ...
    out.writeUInt8(img.size >= 256 ? 0 : img.size, offset + 0); // Width
    out.writeUInt8(img.size >= 256 ? 0 : img.size, offset + 1); // Height
    out.writeUInt8(0, offset + 2); // Color count (0 = >= 8bpp)
    out.writeUInt8(0, offset + 3); // Reserved
    out.writeUInt16LE(1, offset + 4); // Color planes (1)
    out.writeUInt16LE(32, offset + 6); // Bits per pixel (32bpp RGBA)
    out.writeUInt32LE(img.buffer.length, offset + 8); // Size of image data in bytes
    out.writeUInt32LE(currentOffset, offset + 12); // Offset of image data from start of file
...
async function buildFavicons() {
  console.log('Generating official CPL 2026 cricket ball favicons...');

  const publicDir = path.resolve(__dirname, '../public');
  const appDir = path.resolve(__dirname, '../app');

  // 1. Write the vector SVG favicon to public/ and app/
  ...
  console.log('✅ favicon.svg & app/icon.svg written');
  ...
  // Save Apple Touch Icon and PWA icons
  ...
  console.log('✅ apple-touch-icon.png (180x180), icon-192.png, icon-512.png written');

  // 3. Construct a standard multi-resolution ICO file (16x16, 32x32, 48x48)
  ...
  console.log('✅ Multi-size favicon.ico written to public/ and app/');
